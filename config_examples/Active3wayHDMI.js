...
settings.sink = {
    ...
    ecasound: `export LADSPA_PATH=/usr/local/lib/ladspa:/usr/lib/ladspa; stdbuf -i0 -o0 -e0  ecasound -z:mixmode,sum -x -z:nodb -b:ecasound_buffer_size -a:pre1 -f:16,2,44100 -i stdin -pf:pre1-3way.ecp -o:loop,1 -a:pre2,woofer -i:loop,1 -a:pre2 -pf:pre2-3way.ecp -o loop,2 -a:mid,tweeter -i:loop,2 -a:woofer -pf:woofer.ecp -chorder:1,2,0,0,0,0,0,0 -a:mid -pf:mid.ecp -chorder:0,0,1,2,0,0,0,0 -a:tweeter -pf:tweeter.ecp -chorder:0,0,0,0,0,0,1,2 -a:woofer,mid,tweeter -f:s16,8,44100 -o:stdout`,
    outputChannels: 8,
