...
socketAudio.on('message', function (message, remote) {
    ...
    if (messageType == messageTypeJSON) {
        ...
        if (sinkObj.type == 'connectRequest') {
            ...
            socketAudio.send(setuppbuffer, 0, setuppbuffer.length, sinkObj.port, sinkObj.address, function (err, bytes) {
                if (err) throw err;
            });
        }
...
function sendStatusUpdatetoSink() {
    ...
    let setuppbuffer = retrunsetuppbuffer()
    audioSinkList.forEach(function (value, index) {
        socketAudio.send(setuppbuffer, 0, setuppbuffer.length, value.port, value.address, function (err, bytes) {
            if (err) throw err;
        });
    })
}
...
function sendAudioUDP(audioData, sendTime, sampleIndex) {
    if (audioData.length > 0) {
        ...
        if (captureState == 'active') {
            ...
            audioSinkList.forEach(function (value, index) {
                socketAudio.send(sendbuffer, 0, sendbuffer.length, value.port, value.address, function (err, bytes) {
                    if (err) throw err;
                });
            });
